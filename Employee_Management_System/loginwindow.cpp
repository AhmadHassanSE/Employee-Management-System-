    ...
    if (db1.open()){
        ...
        if(query1.exec() && query1.next()){
            ...
            if(query2.exec() && query2.next()){
                ...
                QString id, name, gender, age, department, email, mobile, cnic, experienceInYears;
                int salary, totalLeaves, approvedLeaves, rejectedLeaves;
                id = query2.value("id").toString();
                ...
                Employee emp(username, id, name, age, gender, department, email, mobile, cnic,
                             salary, experienceInYears, totalLeaves, approvedLeaves, rejectedLeaves);
                db1.close();
                Dashboard dash(emp);
                this->hide();
                dash.setModal(true);
